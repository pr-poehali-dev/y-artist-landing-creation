const HeroSection = () => {
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-end justify-center overflow-hidden">
      ...
      {/* Контент — только снизу, кнопки + счётчики */}
      <div className="relative z-20 w-full text-center px-6 pb-16">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
          ...
          <button onClick={() => scroll('nominations')} className="btn-outline-pink px-10 py-4 text-sm rounded-xl"
            style={{ backdropFilter: 'blur(8px)', background: 'rgba(255,42,170,.08)' }}>
            Номинации
          </button>
        </div>
        ...
        <div className="flex gap-12 justify-center">
          {[
            ...
            <div key={s.label} className="text-center">
              <div className="text-3xl font-black text-gold text-glow-gold"
                style={{ textShadow: '0 0 16px rgba(247,201,72,.9), 0 2px 8px rgba(0,0,0,.8)' }}>
                {s.val}
              </div>
              <div className="text-white/55 text-xs tracking-widest uppercase mt-1"
